    ...
    public class WaterproofingHandler : RevitCommandBase, IExternalEventHandler
    {
        ...
        public void Execute(UIApplication app)
        {
            ...
            try
            {
                switch (WaterproofingAction)
                {
                    ...
                    case WaterproofingAction.CreateNewType:
                        CreateWaterproofingFloorType(Context.Doc, WaterproofingLayers);
                        break;
                }
            }
            catch (Exception ex)
            {
                ...
                TaskDialog.Show("Erro no Evento", ex.Message);
            }
            finally
            {
                OnExecuteCompleted?.Invoke();
        ...
        private void RunPickerAndGeneration(UIDocument uidoc, Document doc)
        {
            ...
            IList<Reference> pickedRegionReferences = new List<Reference>();

            try
            {
                PickRegionsService pickRegionsService = new PickRegionsService();
                pickedRegionReferences = pickRegionsService.Pick(Context.UiDoc);
            }
            ...
            if (pickedRegionReferences.Count == 0) return;

            using (Transaction transaction = new Transaction(doc, "Criar Pisos de Impermeabilização"))
            {
                transaction.Start();
                try
                {
                ...
                catch (Exception ex)
                {
                    ...
                    throw new Exception("Falha ao criar os pisos: " + ex.Message);
                }
            }
        }
        ...
        private void CreateWaterproofingFloorType(Document doc, List<WaterproofingLayerItemViewModel> layerItems)
        {

            using (Transaction transaction = new Transaction(doc, "Criar tipo de impermeabilização"))
            {
                transaction.Start();
                try
                {
                    ...
                    FloorType newFloorType = baseFloorType.Duplicate($"IMP_{layerItems[0].Name}") as FloorType;

                    Parameter floorDescParam = newFloorType.get_Parameter(BuiltInParameter.ALL_MODEL_DESCRIPTION);
                    if (floorDescParam != null && !floorDescParam.IsReadOnly)
                    {
                        floorDescParam.Set(layerItems[0].Name.ToUpper());
                    }
                    ...
                    {
                        string displayName = layerItem.Name;
                        string internalFloorName = $"IMP_{layerItem.Name}";
                        string internalMaterialName = $"IMP_{layerItem.Name.Replace(" ", "_")}";

                        ElementId newMaterialId = Material.Create(doc, internalMaterialName);
                        Material newMaterial = doc.GetElement(newMaterialId) as Material;

                        newMaterial.MaterialClass = "Waterproofing";
                        ...
                        Parameter paramVertical = newMaterial.LookupParameter("PRJ IMP_Multiplicador_Vertical");
                        ...
                        MaterialFunctionAssignment layerFunction = MaterialFunctionAssignment.Structure;
                    ...
                    newCompoundStructure.SetNumberOfShellLayers(ShellLayerType.Interior, 0);
                    newCompoundStructure.SetNumberOfShellLayers(ShellLayerType.Exterior, 0);
                    newCompoundStructure.EndCap = EndCapCondition.NoEndCap;

                    // Aplica a nova estrutura ao piso duplicado
